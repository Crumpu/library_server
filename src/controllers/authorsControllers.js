  ...
  async getAuthors(req, res) {
    try {
      const authors = await db.query(
        `SELECT authors.id, full_name, email, nationalities.title, authors.created_at, authors.updated_at FROM authors
  ...
  async createAuthor(req, res) {
    try {
      ...
      const newAuthor = await db.query(
        `INSERT INTO authors(full_name, email, nationality_id, "created_at")
        VALUES 
        ($1, $2, (SELECT id FROM nationalities WHERE title=$3), $4)
        RETURNING *`,
        [full_name, email, nationality, created_at]
      );
  ...
  async updateAuthor(req, res) {
    try {
      ...
      const updatedAuthor = await db.query(
        `UPDATE authors 
        SET 
        full_name=$1, email=$2, nationality_id=(SELECT id FROM nationalities WHERE title=$3), "updated_at"=$4
        WHERE id=$5
        RETURNING *`,
        [full_name, email, nationality, updated_at, id]
      );
      if (updatedAuthor.rows.length > 0) {
        ...
        res
          .status(404)
          .send(`Author can't updated, error is: ${error.message}`);
      }
    } catch (error) {
